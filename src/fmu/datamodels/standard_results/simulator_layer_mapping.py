...
class SimulatorLayerMappingResultRow(BaseModel):
    """Represents the columns of a row in a simulator layer mapping export.
    ...
    """Index column. The layer index this row represents (1-based). Required."""

    ZONE: str
    """Index column. The zone that the layer corresponds to. Required."""


class SimulatorLayerMappingResult(RootModel):
    """Represents the resultant simulator layer mapping parquet file, which is
    ...
    root: list[SimulatorLayerMappingResultRow]


class SimulatorLayerMappingSchema(SchemaBase):
    ...
    FILENAME: str = "simulator_layer_mapping.json"
    ...
    def dump(cls) -> dict[str, Any]:
        return SimulatorLayerMappingResult.model_json_schema(
